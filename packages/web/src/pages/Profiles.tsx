...
import type { Client, ProfileDetail, PublicProxy, Session } from '../types.js';
    ...
    >
      ...
      >
        ...
        <dl className="mb-6 grid grid-cols-2 gap-x-6 gap-y-2 text-sm">
          ...
          <dd><StateBadge state={profile.state} /></dd>
          <dt className="text-zinc-500">Client</dt>
          <dd>{clientName}</dd>
          <dt className="text-zinc-500">Proxy</dt>
          <dd>{profile.proxy ? `${profile.proxy.name} (${profile.proxy.scheme}://${profile.proxy.host}:${String(profile.proxy.port)})` : 'none'}</dd>
          ...
          <dd>
            {profile.lock.active ? `held by ${profile.lock.owner ?? 'unknown'}` : 'free'}
          </dd>
        ...
        <div className="mb-6 flex flex-wrap gap-2">
          <Button variant="primary" disabled={busy} onClick={() => void mutate(() => client.launchProfile(profile.id))}>
            Launch
          </Button>
          <Button disabled={busy} onClick={() => void mutate(() => client.stopProfile(profile.id))}>
            Stop
          </Button>
          <Button disabled={busy} onClick={() => void mutate(() => client.restartProfile(profile.id))}>
            Restart
          </Button>
        </div>
        ...
        <div className="mb-6">
          ...
          <div className="flex gap-2">
            ...
            {profile.proxy && (
              <Button disabled={busy} onClick={() => void mutate(() => client.unassignProxy(profile.id))}>
                Unassign
              </Button>
            )}
          </div>
        ...
        <BackupList client={client} profileId={profile.id} profileName={profile.name} onChanged={onChanged} />
      </div>
    </div>
  );
}
...
export function ProfilesPage({
  ...
  return (
    <div>
                ...
                <tr key={p.id} className="border-t border-zinc-800 hover:bg-zinc-900/60">
                  ...
                  <td className="px-4 py-2">
                    <div className="flex flex-wrap gap-1">
                      ...
                      <Button variant="ghost" onClick={() => { setDetailId(p.id); }}>
                        Details
                      </Button>
                    </div>
                  </td>
      ...
      {showCreate && (
        <Modal title="New profile" onClose={() => { setShowCreate(false); }}>
          <form
            className="space-y-4"
            onSubmit={(e) => {
              e.preventDefault();
